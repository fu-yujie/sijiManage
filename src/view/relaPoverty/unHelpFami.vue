        ...
        <ul id="ul1">
          <li v-for = " (item,index) in famiListBox " :key = "index">
            ...
            <div class="famiListBtn">
				<p @click="addHelp(index)" class="specialBtn">
                	添加绑定
              	</p>
...
import {
	GetPoorFamilyNoRelScope,
	GetPoorFamilyNoRelList,//所有未绑定信息
	AddPoorLeaderRelation,//添加绑定
} from "../../api";
...
export default {
	...
	// 	next(vm => {
	// 		let fromRou = from.name;
	// 		if(fromRou!==null){
	// 			if( fromRou.indexOf("helpFamiMem") > -1 ){
	// 				debugger;
	// 				let arrs = sessionStorage.getItem("searchKeysArr").split("/,");
	// 				let keys = sessionStorage.getItem("searchKeys");
	// 				if(!keys){ keys="" };
	// 				vm.PageIndex = arrs[0];
	// 				vm.areaCode = arrs[1];
	// 				vm.townCode = arrs[2];
	// 				vm.streetCode = arrs[3];
	// 				vm.NameOrIDCard = keys;
	// 				console.log( vm.NameOrIDCard );
	// 				vm.GetPoorFamilyNoRelList();
	// 			};
	// 		}
	// 	})
	// },
	...
	// 	let toRou = to.name;
	// 	if(toRou!==null){
	// 		if( toRou.indexOf("helpFamiMem") > -1 ){
	// 			let arrs=[];
	// 			arrs[0] = this.PageIndex;
	// 			arrs[1] = this.areaCode;
	// 			arrs[2] = this.townCode;
	// 			arrs[3] = this.streetCode;
	// 			sessionStorage.setItem("searchKeysArr", arrs);
	// 			sessionStorage.setItem( "searchKeys", this.NameOrIDCard );
	// 		};
	// 	};
	// 	next();
	// },
	...
	methods: {
		...
		try {
			let res = await GetPoorFamilyNoRelList(Entity);
			Indicator.close();
			console.log(res);
		...
		//获取区域
		async GetPoorFamilyNoRelScope() {
		let Par = {
			OperatorId: this.OperatorID
		};
		try {
			let res = await GetPoorFamilyNoRelScope(Par);
		...
		//添加绑定的ajax事件
		async AddPoorLeaderRelation( liIndex ){
			let parama = {
				PoorFamilyNum: this.famiListBox[liIndex].FamilyId,
				OperatorId: this.OperatorID,
			}
			try{
				let res = await AddPoorLeaderRelation( parama );
				console.log( res );
				let resInfo = res.data.Info;
				let resCode = res.data.StatusCode;
				Toast({
		...
		addHelp( liIndex ){
			...
			MessageBox.confirm(
				'点击“确定”按钮将建立与贫困户 '+ famiName +' 一家（编号：'+ famiNum +'）的帮扶关系，点击“取消”按钮可取消操作。',
				'确定绑定'
				).then(action => {
				this.AddPoorLeaderRelation( liIndex );
